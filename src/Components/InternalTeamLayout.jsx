...
const InternalTeamLayout = ({ children }) => {
  ...
  const getUserDataSync = () => {
    try {
      ...
      if (userStr) {
        ...
        return {
          name: name || "Internal Team",
          email: email || "staff@vettedpool.com"
        };
      }
    } catch (e) {
  ...
  const getUserInitial = () => {
    if (displayName && displayName !== "Internal Team") {
      // Get first letter and make it uppercase
      return displayName.charAt(0).toUpperCase();
    }
    // Fallback to "I" if no valid name found
    return "I";
  };
  ...
  // Load user display info from localStorage
  useEffect(() => {
    try {
      const userStr = localStorage.getItem('user');
      if (userStr) {
